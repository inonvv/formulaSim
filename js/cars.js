...
// vMax [km/h] — realistic attainable top speed per car. main.js clamps the
// speed target to the active car's vMax (slider max stays 350 globally).
const CAR_META = {
  F1: { label: 'Formula One',   color: 0xe8132a, vMax: 350 },
  F2: { label: 'Formula Two',   color: 0x2a6fdb, vMax: 335 },
  F3: { label: 'Formula Three', color: 0x00a86b, vMax: 300 },
  GT: { label: 'GT Race Car',   color: 0xff8800, vMax: 310 },
};
...
export async function buildCar(type) {
  ...
  switch (type) {
    case 'F2': return buildF2Procedural(meta);   // procedural-only (no GLB)
    case 'F3': return buildF3Procedural(meta);   // procedural-only (no GLB)
    case 'GT':
      return (flag?.GT === true)
        ? buildGTHybrid(meta)
 ...
 */
function measureFromWheels(wPos, wR, wW) {
  return {
...
function buildF2Procedural({ color }) {
  ...
  /* ── DIFFUSER ─────────────────────────────────────────────── */
  const diffF2 = mesh(box(1.00, 0.054, 0.88), matCarbon, 0, -0.042, 1.80);
  diffF2.rotation.x = -0.24;
  grp.add(diffF2);
  ...
  const measure = measureFromWheels(wPos, wR, wW);
...
function buildF3Procedural({ color }) {
  ...
  /* ── DIFFUSER ─────────────────────────────────────────────── */
  const diffF3 = mesh(box(0.88, 0.048, 0.76), matCarbon, 0, -0.038, 1.68);
  diffF3.rotation.x = -0.22;
  grp.add(diffF3);
  for (let di = -1; di <= 1; di++) {
